...
export function REPORTSOFTHEACADEMYOFSCIENCESOFTHEREPUBLICOFTATARSTAN() {
  const { items } = useSelector((state: RootState) => state.Docx);

  return (
    <section className="about-unior in-main">
      {items
        .filter((o, i) => o.positions === pwdDocx.REPORTSOFTHEACADEMYOFSCIENCESOFTHEREPUBLICOFTATARSTAN)
        .map((k, j) => (
